...
@interface HT_Detail_ViewController ()
{
...
@implementation HT_Detail_ViewController
...
- (void)viewDidLoad
{
    ...
    __block HT_Detail_ViewController * weakSelf  = self;
...
- (void)didRequestData
{
    NSString * uri = [NSString stringWithFormat:@"%@?page=%i&count=%@", url, pageNo, maxR];
    
    [[LTRequest sharedInstance] didRequestInfo:@{@"absoluteLink":[uri encodeUrl],@"method":@"GET",@"overrideError":@(1),@"host":self,@"overrideLoading":@(1)} withCache:^(NSString *cacheString) {
...
- (void)didPressMenu:(UIButton*)sender
{
    ...
    if([[self autoIncrement:@"menu456"] intValue] % 5 == 0)
    {
        [self performSelector:@selector(presentAds) withObject:nil afterDelay:0.5];
    }
}

- (IBAction)didPre:(id)sender
{
    [APPDELEGATE changeRoot:NO];
}
...
- (void)tableView:(UITableView *)_tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([[self autoIncrement:@"detail123"] intValue] % 4 == 0)
    {
        [self performSelector:@selector(presentAds) withObject:nil afterDelay:0.5];
    }
